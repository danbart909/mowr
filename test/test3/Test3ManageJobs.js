...
export default class Test3ManageJobs extends Component {
  constructor(props) {
    super(props)
    this.state = {
      busy: false,
  ...
  renderList = () => {
    ...
    // .description
    // dFNS.formatDistance(new Date(item.creationDate), new Date(), { addSuffix: true })
    // dFNS.formatDistance(new Date(item.endDate), new Date(), { addSuffix: true })
    // this.viewJob(x)
